...
crosstab_result <- anes2024 %>% #name the dataframe that you are using
  ...
  filter(!is.na(income)) %>% 
  tabyl(income, education) %>% #put the dependent variable first
...
crosstab_result
```
**Remember when interpreting this output that the numbers are column percents, and the raw numbers are in parentheses.** If you take a look at this output, you will see that people with more education seem to make more money. If you look at the top income category row (for those with household incomes of \$250,000 or more, the percent of those at the lowest education level, "less than high school" in that income category is 0.39%. As the education level increases this percent increase to 2.42%, 4.27%, 8.49%, and 18.18% for those with graduate degrees.
