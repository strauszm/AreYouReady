...
We are going to install three new packages to use today.They are called "Hmisc", "flextable", and "janitor". Enter these commands into your Console one by one:
```
...
library(janitor)      #this helps us make nice crosstabs
...
#third, we generate a new factor called liberalism with the opposite levels as our ideology variable 
anes2024$liberalism <- factor(anes2024$ideology, levels = v)
...
Take a look at the above column totals. Our anes2024 dataframe has 1,034 people with graduate degrees, while it has 1,296 people with BAs. So, returning to those two numbers from above, the 488 people with graduate degrees with household incomes of \$100,000 to \$250,000 make up 47.2% of all of graduate degree holders in our sample (because 488/1034=.472), while the 548 people with BAs with household incomes of \$100,000 to \$250,000 make up only 42.3% of all of the people with some college in our sample (because 528/1296=.423). In other words, there is a larger proportion of people with graduate degrees with household incomes of \$100,000 to \$250,000 than people with BAs with household incomes of \$100,000 to \$250,000. **Thus, when interpreting crosstabs it is important to look at the column percents, and not just the raw numbers.** To make R generate all of the column percents in a crosstab at the same time, we can use the following code from the janitor package that we downloaded for today:
```{r}
crosstab_result <- anes2024 %>% #name the dataframe that you are using
  filter(!is.na(education)) %>% 
  filter(!is.na(income)) %>% 
  tabyl(education, income) %>% #put the independent variable first
  adorn_percentages("col") %>%
  adorn_totals("col") %>%
  adorn_totals("row") %>%
  adorn_pct_formatting(digits = 2) %>% #round your numbers
  adorn_ns()
crosstab_result
```
**Remember when interpreting this output that the numbers are column percents, and the raw numbers are in parentheses.** If you take a look at this output, you will see that people with more education seem to make more money. If you look at the top income category row (for those with household incomes of \$250,000 or more, the percent of those at the lowest education level, "less than high school" in that income category is 0.26%. As the education level increases this percent increase to 5.63%, 17.9%, 28.13%, and 48.08% for those with graduate degrees.

### Exporting cross-tabs for your word processor {#export-crosstabs}
If you have been following along, the cross-tabs that you have generated have shown up reasonably neatly in the Console of R-Studio. But often, you will want to export those tables into a format that you can easily bring into your word processor so that you can refer to it in papers that you are writing. In section \@ref(column-percents), we named the crosstab with education in rows and income in columns ```crosstab_result```. To export that crosstab, you can use the following code:

```{r}
flextable(crosstab_result) %>% save_as_docx(path="crosstab_result.docx")
```
This has created a file called "crosstab_result.docx" in the main folder where you have your Scope and Methods labs (the folder that you created in section \@ref(class-project)). You can open that file in Microsoft Word, or you can pull it into a Pages file or a Google Document. If you'd rather export your table into a rich text format file, you can use ```save_as_rtf``` instead of ```save_as_docx``` in the above command.
...
r4<-c("as.numeric", "You can use this command to make a factor into a numeric variable, like this: df\\$NewVariable<-as.numeric(df\\$Variable)","Base R")
r5<-c("case_when()","You can use this command along with mutate() and %in% to assign ranges of an interval variable to a new value of a new variable","dplyr (tidyverse)")
r6<-c("ordered","you can use this command to make a variable into an ordered factor", "Base R")
r7<-c("xtabs()","Makes a crosstab table with raw values. Remember to define variables like this: xtabs(~DV+IV, data=DATAFRAME)","Base R")
r8<-c("addmargins(xtabs())","Makes a crosstab table with row and column totals.","Base R")
