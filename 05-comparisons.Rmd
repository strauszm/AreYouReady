...
Do we think that this result would be roughly the same for men and women? To answer this question, we can use a neat tool that we have actually played with a bit in previous chapters: the "filter" command that is a part of the dplyr package (and thus works with pipes). The ANES dataset has a variable called female, where 0 means "men" and 1 means "women." Let’s have R make that crosstab again, but only for men, and then again, but only for women. To do that, we can insert the qualifier ``` filter(female==[THE VALUE WE WANT])%>%``` after we call our data. Like this:
```{r}
#men only
men_crosstab <- anes2024 %>% #name the dataframe that you are using
  filter(female==0)%>% #only look at men this time
...
```{r}
flextable(men_crosstab) %>% save_as_docx(path="men_crosstab.docx")
```

Now let’s look at women only:
```{r}
#women only
women_crosstab <- anes2024 %>% #name the dataframe that you are using
  filter(female==1)%>% #only look at men this time
  filter(!is.na(education)) %>% 
  filter(!is.na(income)) %>% 
  tabyl(income, education) %>% #put the dependent variable first
  adorn_percentages("col") %>%
...
```{r}
flextable(women_crosstab) %>% save_as_docx(path="women_crosstab.docx")
```
